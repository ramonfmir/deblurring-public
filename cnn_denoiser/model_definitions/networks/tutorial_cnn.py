...
def autoencoder(inputs, batch_size):
    # Encoder
    net = lays.conv2d(inputs, 64, [5, 5], stride=3, padding='SAME')
    ...
    # print(net.shape)
    net = lays.conv2d(net, 32, [30, 15], stride=10, padding='SAME')
    print(net.shape)
    tf.summary.image('code_layer', tf.expand_dims(tf.transpose(net, [3, 0, 1, 2])[0], 3), max_outputs=1)

    # Decoder
    net = lays.conv2d_transpose(net, 64, [30, 15], stride=10, padding='SAME')
    ...
    net = lays.conv2d(net, channels, [5, 5], stride=1, padding='SAME')
    print(net.shape)
